...
Память представляет из себя два раздела:
...
* `DATA` - раздел, в котором хранятся переменные, строки и массивы, созданные в файле с секцией "section .data" подряд в том же порядке, в котором они перечислены в коде.

* `INSTR` - раздел, в котором хранятся инструкции, описанные в файле с секцией "section .text".
...
Некоторые инструкции могут принимать более двух операндов. В таком случае при трансляции сложная инструкция преобразуется в N простых.
...
Интерфейс командной строки: `translator.py <input_text_file> <input_data_file> <target_text_file> <target_data_file>`
...
На вход принимает четыре файла:
* Файл с программой на языке высокого уровня, файл с данными.
* Путь к файлу, в который будет записана программа в машинных словах, путь к файлу, куда будет записана бинарная память
...
section .data
    ...
    name: "Hello"                               ; создается массив первый, нуль-терминированная строка
...
## Модель процессора
Интерфейс командной строки: `mashine.py <machine_code_file> <data_bin_file> <input_file>`
...
* `data` - данные из памяти, которые передается в CU
